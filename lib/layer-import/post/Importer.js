                ...
                input.pipe(ndjson.parse({strict: true}))
                     ...
                     .on('data',function(feature){
                         var def = q.defer(),
                             ...
                             retryQs = [1e2,2.5e2,0.5e3,1e3,1.5e3];
                        ...
                        function tryAddFeature(featureData) {
                            (new Feature({
                               ...
                               if(err) {
                                   ...
                                   def.resolve();
                                   /* this kind of simplification seems to have no effect on duplicate vertices
                                    * need to consider another approach or ... live with it.
                                   if(retryQs.length) {
                                       var q = retryQs.pop(),topo,geo,featureCopy,objects;
                                       self.emit('info','Retrying simplified geometry with q of '+q);
                                       try {
                                           featureCopy = JSON.parse(JSON.stringify(feature));
                                           //fs.createWriteStream('/tmp/'+featureId+'.ndjson').end(JSON.stringify(featureCopy));
                                           objects = {l: {type: 'FeatureCollection',features:[featureCopy]}};
                                           topo = topojson.topology(objects,q);
                                           geo = topojson.feature(topo,topo.objects.l);
                                           tryAddFeature(geo.features[0]);
                                       } catch(simp_err) {
                                           console.error(simp_err);
                                           self.emit('info','ERROR: '+JSON.stringify(simp_err));
                                           return def.resolve();
                                       }
                                   } else {
                                       self.emit('info','FAILED: retries exceeded');
                                       def.resolve();
                                   }*/
                               } else {
